...
from core.datasets import VOC_Dataset_For_Classification, VOC_Dataset_For_Testing_CAM, Iterator
...
from tools.ai.log_utils import Average_Meter
from tools.ai.demo_utils import denormalize, colormap
from tools.ai.optim_utils import PolyOptimizer
from tools.ai.torch_utils import set_seed, L1_Loss, L2_Loss, shannon_entropy_loss, make_cam, calculate_parameters, get_learning_rate_from_optimizer, get_numpy_from_tensor, load_model, save_model, save_checkpoint, load_ckp
from tools.ai.evaluate_utils import Calculator_For_mIoU

from tools.ai.log_utils import log_print
from tools.ai.augment_utils import RandomCrop, RandomHorizontalFlip, RandomResize, Normalize, Normalize_For_Segmentation, Top_Left_Crop_For_Segmentation, Transpose, Transpose_For_Segmentation
...
if __name__ == '__main__':
    ...
    best_train_mIoU = -1
    thresholds = list(np.arange(0.10, 0.50, 0.05))      # for bg thresholding
            ...
            for step, (images, labels, gt_masks) in enumerate(loader):
                ...
                if step == 0:
                    ...
                    for b in range(2):
                        ...
                        writer.add_image('CAM/{}'.format(b + 1), image, global_counter, dataformats='HWC')
    ...
    start_iter = args.global_counter

    for global_counter in range(start_iter, max_iteration):
        ...
        if args.alpha_schedule == 0.0:
            ...
            alpha = min(args.alpha * global_counter / (max_iteration * args.alpha_schedule), args.alpha)
        
        loss = loss + class_loss + p_class_loss + alpha * re_loss + conf_loss
        #################################################################################################

        if (global_counter+1) % 16 == 0:
            # loss = loss_local
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if (global_counter + 1) % log_iteration == 0:
            ...
            writer.add_scalar('Train/loss', loss, global_counter)
            writer.add_scalar('Train/class_loss', class_loss, global_counter)
            writer.add_scalar('Train/p_class_loss', p_class_loss, global_counter)
            writer.add_scalar('Train/re_loss', re_loss, global_counter)
            writer.add_scalar('Train/conf_loss', conf_loss, global_counter)
            writer.add_scalar('Train/learning_rate', learning_rate, global_counter)
            writer.add_scalar('Train/alpha', alpha, global_counter)
        
        #################################################################################################
        # Evaluation
        #################################################################################################
        if (global_counter + 1) % val_iteration == 0:
            current_epoch = global_counter // val_iteration
            ...
            if best_train_mIoU == -1 or best_train_mIoU < mIoU:
                ...
                save_checkpoint({
                            ...
                            savepath=f"voc12_epoch_{current_epoch}.pth")
                log_func('[i] saved cpk')
            ...
            writer.add_scalar('Evaluation/threshold', threshold, global_counter)
            writer.add_scalar('Evaluation/train_mIoU', mIoU, global_counter)
            writer.add_scalar('Evaluation/best_train_mIoU', best_train_mIoU, global_counter)
