...
import torchvision
from utils import imutils, pyutils, iouutils
import argparse
...
import cv2
from utils import infer_utils




cam_20_list = []                # 20 classes ?
cam_200_list = []               # what is 200, 20*10 sub categories
...
if __name__ == '__main__':
    ...
    parser.add_argument("--low_alpha", default=4, type=int, help="crf parameter")           # opacity parameters for alpha
    parser.add_argument("--high_alpha", default=24, type=int, help="crf parameter")
    ...
    infer_utils.create_folder(args.save_path)           # create folders if they don't exist

    model = getattr(importlib.import_module(args.network), 'Net')(args.k_cluster, args.round_nb)        # get model .py file
    model.load_state_dict(torch.load(args.weights))                                                     # load state dict

    model.eval()
    model.cuda()

    infer_dataset = voc12.data.VOC12ClsDatasetMSF(args.infer_list, voc12_root=args.voc12_root,          # get dataset from list
    ...
    n_gpus = torch.cuda.device_count()          # use as many gpus as you have
    ...
    for iter, (img_name, img_list, label) in enumerate(infer_data_loader):
        ...
        orig_img = np.asarray(Image.open(img_path))             # get original img
        orig_img_size = orig_img.shape[:2]                      # and size

        gt_cat = np.where(label==1)[0]                          # get ground truth label class
        label_200 = torch.zeros((20*args.k_cluster))            # 20 classes * 10 sub categories
        # label_200 = torch.zeros((args.k_cluster))
        for cat in gt_cat:
            label_200[cat*args.k_cluster:cat*args.k_cluster+args.k_cluster] = 1         # make label matrix
        ...
        def _work(i, img):
            with torch.no_grad():
                with torch.cuda.device(i%n_gpus):
                    cam, cam_200 = model_replicas[i%n_gpus].forward_two_cam(img.cuda())         # get cam and then upsample it
