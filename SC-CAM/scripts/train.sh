...
R1_extractor_model=./pretrained_model/res38_cls.pth
...
## R2
# Extract the feature
#python extract_feature.py --weights ${R2_extractor_model} --infer_list ./voc12/train_aug.txt --voc12_root 
#${dataset_folder} --save_folder ${save_folder} --k_cluster 10 --from_round_nb 1
# Generate the pseudo label
#python create_pseudo_label.py --save_folder ${save_folder} --k_cluster 10 --for_round_nb 2
# Train the classifier
#python train_cls.py --lr 0.01 --voc12_root ${dataset_folder} --weights ${pretrained_model} --round_nb 2 
...
python ./scripts/infer_cls.py \
        --infer_list voc12/train.txt \
        --voc12_root ${dataset_folder} \
        --weights ${final_model} \
        --save_path ${save_cam_folder} \
        --save_out_cam 1 \
        --k_cluster 10 \
