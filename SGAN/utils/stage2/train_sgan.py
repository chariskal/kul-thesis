...
from lib.dataset.get_dataset import get_dataset         # custom py scripts
...
if __name__ == '__main__':
    ...
    device = torch.device("cuda:0")         # gpu device 0
    cur_time = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())         # get time
    pyutils.Logger(osp.join("log", cur_time + ".log"))                      # set logger
