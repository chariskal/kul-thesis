...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_root", default='default="/esat/izar/r0833114/VOCdevkit/VOC2012', type=str)
    parser.add_argument("--la_crf_dir", required=True, type=str)
    parser.add_argument("--ha_crf_dir", required=True, type=str)
    parser.add_argument("--snapshot_dir", default="./checkpoints/train_aff/", type=str)
    ...
    pyutils.Logger(args.snapshot_dir + args.session_name + '.log')
    print(vars(args))
    ...
    train_dataset = voc12.data.VOC12AffDataset(args.train_list, label_la_dir=args.la_crf_dir, label_ha_dir=args.ha_crf_dir,
                                               voc12_root=args.data_root, cropsize=args.crop_size, radius=5,
    ...
    for ep in range(args.max_epoches):
        for iter, pack in enumerate(train_data_loader):
            ...
            if (optimizer.global_step - 1) % 50 == 0:
                ...
                avg_meter.pop()
        if ep % 2 == 0:
            torch.save(model.module.state_dict(), f'{args.snapshot_dir}_{args.session_name}_checkpoint_{ep}.pth')
            print(f'Loss: {loss} at Epoch {ep} weights saved!')
