...
if __name__ == '__main__':
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        cam = np.load(os.path.join(args.cam_dir, name + '.npy'), allow_pickle=True).item()          # load cam dict
        cam_full_arr = np.zeros((21, orig_shape[2], orig_shape[3]), np.float32)                     # init 21xHxW array
        for k, v in cam.items():
            cam_full_arr[k+1] = v                                                                   # replace CAMs in array
        # for BG class set everything to a constant value (?) which changes final segmentation
        ...
        with torch.no_grad():
            aff_mat = torch.pow(model.forward(img.cuda(), True), args.beta)             # model output^beta

            trans_mat = aff_mat / torch.sum(aff_mat, dim=0, keepdim=True)               # normalize
            for _ in range(args.logt):
                trans_mat = torch.matmul(trans_mat, trans_mat)                          # do matrix multiplication logt times

            cam_full_arr = torch.from_numpy(cam_full_arr)       # to array
...
#                cam_full_arr = torch.from_numpy(cam_full_arr).view(1, 21, dheight, dwidth).cuda()
            ...
            if args.crf:    # improve segmentation result w/ CRF
            ...
            # final pixel level segmentation is where class is maximum
